...
def arxiv():
    ...
    #get labels
    labels=data.y
    np.savez(root_folder+'/data/arxiv/arxiv_labels_all.npz', labels = labels)
    exit(0)
...
def tmall():
    ...
    zeros_tensor = torch.zeros(data.num_nodes - data.y.size(0), 1, dtype=data.y.dtype)

    # Concatenate the existing labels with the zeros tensor
    labels = torch.cat((data.y, zeros_tensor), dim=0)

    np.savez(root_folder+'/data/tmall/tmall_labels_all.npz', labels = labels)
    # labels = labels.numpy()
    ...
    exit(0)
    # For initial graph
    # row,col,time = data.edge_index[0]
    
    
    # row_tensor = torch.from_numpy(row.astype(np.int64))
    # col_tensor = torch.from_numpy(col.astype(np.int64))
    edge_number = 0
    # initial_edges = torch.cat((row_tensor, col_tensor), dim=0)
    row, col =to_undirected(edge_index = data.edge_index[0],num_nodes = data.num_nodes)
        ...
        if(self_loop):
            ...
            for i, j in zip(row, col):
                if(i != j):
                    ...
                    f.write("%d %d\n" % (j, i))
                    edge_number+=1
    print("initial edge_number", edge_number) 
    save_adj(row, col, N=data.num_nodes, dataset_name='tmall', savename='tmall_init', snap='init')


    print("len(data.edge_index)", len(data.edge_index))
    print("data.edge_index[0].shape", data.edge_index[0])
    edge_number = 0
    for sn in range(len(data.edge_index)-1):
        print(sn)
        row,col=data.edge_index[sn]
        
        file_name = root_folder+'/data/tmall/tmall_Edgeupdate_snap' + str(sn+1) + '.txt'
...
def dblp():
    ...
    labels=data.y
    np.savez(root_folder+'/data/dblp/dblp_labels_all.npz', labels = labels)
    # labels = labels.numpy()
    # labels = np.array(labels,dtype=np.int32)
    # labels = labels.T
    # labels=labels.reshape(labels.shape[1])
    ...
    np.savez(root_folder+'/data/dblp/dblp_labels.npz',train_idx=train_idx,val_idx=val_idx,test_idx=test_idx,train_labels=train_labels,val_labels=val_labels,test_labels=test_labels)
    ...
    # initial_edges = torch.cat((row_tensor, col_tensor), dim=0)
    row, col =to_undirected(edge_index = data.edge_index[0],num_nodes = data.num_nodes)
    with open(root_folder+'/data/dblp/dblp_init_adj' + '.txt', 'w') as f:
        if(self_loop):
            for i, j in zip(row, col):
                f.write("%d %d\n" % (i, j))
                f.write("%d %d\n" % (j, i))
            ...
            for i, j in zip(row, col):
                if(i != j):
                    ...
                    edge_number+=1
    print("initial edge_number", edge_number) 
    save_adj(row, col, N=data.num_nodes, dataset_name='dblp', savename='dblp_init', snap='init')
    ...
    for sn in range(len(data.edge_index)-1):
        print(sn)
        row,col,time=data.edge_index[sn]
        
        file_name = root_folder+'/data/dblp/dblp_Edgeupdate_snap' + str(sn+1) + '.txt'
        with open(file_name, 'w') as f:
            if(self_loop):
                for i, j, t in zip(row, col, time):
                    f.write("%d %d\n" % (i, j))
                    f.write("%d %d\n" % (j, i))
                    edge_number+=2
            else:
                for i, j, t in zip(row, col, time):
                    if(i != j):
                        ...
                        edge_number+=2
        print("edge_number", edge_number)
    print('dblp -- save snapshots finish')
...
def patent():
    ...
    labels=data.y
    np.savez(root_folder+'/data/patent/patent_labels_all.npz', labels = labels)
    exit(0)
    train_labels=labels.data[train_idx]
    val_labels=labels.data[val_idx]
    test_labels=labels.data[test_idx]
...
def reddit():
    ...
    labels=data.y
    np.savez(root_folder+'/data/reddit/reddit_labels_all.npz', labels = labels)
    exit(0)
    train_labels=labels.data[train_idx]
    val_labels=labels.data[val_idx]
    test_labels=labels.data[test_idx]
